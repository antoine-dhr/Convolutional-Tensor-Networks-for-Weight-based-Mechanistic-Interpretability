...
def plot_explanation(model, sample: Float[Tensor, "w h"], eigenvalues=10):
    ...
    elif sample_input.dim() == 2:
        sample_input = sample_input.unsqueeze(0).unsqueeze(0)

    logits = model(sample_input)[0].detach().cpu()
    ...
    sample_vec = sample_input
    if sample_vec.dim() == 4:
        sample_vec = sample_vec[0]
    sample_flat = sample_vec.flatten().cpu()

    vecs_flat = vecs.flatten(start_dim=2) if vecs.dim() > 3 else vecs
    acts = einsum(sample_flat, vecs_flat, "inp, cls comp inp -> cls comp").pow(2) * vals
    ...
    sample_image = sample
    if sample_image.dim() == 4:
        sample_image = sample_image[0, 0]
    elif sample_image.dim() == 3:
        sample_image = sample_image[0]

    fig.add_heatmap(z=sample_image.flip(0).cpu(), colorscale="RdBu", zmid=0, showscale=False, row=1, col=5)
