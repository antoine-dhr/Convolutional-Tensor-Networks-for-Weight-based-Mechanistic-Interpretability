...
class BilinearConv2d(nn.Conv2d):
    """A bilinear 2D convolution with optional gate"""
    def __init__(self, in_channels: int, out_channels: int, kernel_size=3, padding=None, bias=False, gate=None) -> None:
        if padding is None:
            if isinstance(kernel_size, tuple):
                padding = tuple(size // 2 for size in kernel_size)
            else:
                padding = kernel_size // 2
        super().__init__(in_channels, 2 * out_channels, kernel_size=kernel_size, padding=padding, bias=bias)
        self.gate = {"relu": nn.ReLU(), "silu": nn.SiLU(), "gelu": nn.GELU(), None: nn.Identity()}[gate]

    def forward(self, x: Float[Tensor, "... c h w"]) -> Float[Tensor, "... c h w"]:
        left, right = super().forward(x).chunk(2, dim=1)
        return self.gate(left) * right
